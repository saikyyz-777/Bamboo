-- Build a Bamboo Factory Hub v2.4
-- AutoCollect | AutoBuy | AutoCash | Config (TPWalk, InfJump, AntiAFK, Presets)
-- Loading screen + Notificacoes + Tamanho igual ao MultipleChoices
-- v2.4: Limpeza de animacoes/redundancias, toggles otimizados
...
local AUTOLOAD_FILE      = "drhub_autoload.txt"  -- v2.3: qual preset carrega automaticamente
...
-- ════════════════════════════════════════════════════════════════
-- PROFILE SYSTEM (v2.3: inclui estado dos toggles)
-- ════════════════════════════════════════════════════════════════
...
function ProfileSystem:createProfile(name)
    ...
    -- v2.3: estado dos toggles incluido no preset padrao
    self.profiles[name] = {
...
local lsGrad = Instance.new("UIGradient", loadScreen)
lsGrad.Color = ColorSequence.new{
...
local lsTitleG = Instance.new("UIGradient", lsTitle)
lsTitleG.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0,   Color3.fromRGB(60,220,120)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255,255,255)),
    ColorSequenceKeypoint.new(1,   Color3.fromRGB(60,220,210)),
}
...
lsVer.Text = "v2.4  |  AutoCollect | AutoBuy | AutoCash"; lsVer.TextTransparency = 1
...
local lsBar = Instance.new("Frame", lsBarBg)
...
spinnerFrame.Position = UDim2.new(0.5, 0, 0.5, 140)
spinnerFrame.BackgroundTransparency = 1
spinnerFrame.ZIndex = 1001
...
spinnerGrad.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, C.accent_green),
    ColorSequenceKeypoint.new(0.5, C.accent_green),
    ColorSequenceKeypoint.new(0.51, Color3.new(0,0,0)),
    ColorSequenceKeypoint.new(1, Color3.new(0,0,0))
}

-- Spinner rotation (loading screen only)
RunService.Heartbeat:Connect(function()
    if spinnerGrad and spinnerGrad.Parent then
        spinnerGrad.Rotation = (spinnerGrad.Rotation + 10) % 360
    end
end)
...
task.spawn(function()
    ...
    while lsLogoI.Parent do
        tw(lsLogoF, {Size = UDim2.new(0, 110, 0, 110), BackgroundTransparency = 0.4}, 0.8, Enum.EasingStyle.Sine)
        task.wait(0.8)
        tw(lsLogoF, {Size = originalSize, BackgroundTransparency = 0.75}, 0.8, Enum.EasingStyle.Sine)
        task.wait(0.8)
    end
end)
...
local function notify(title, msg, dur, col)
    ...
    for _, n in ipairs(notifStack) do
        tw(n, {Position=UDim2.new(1,-360,0,n.Position.Y.Offset-90)}, 0.3)
    end
    local f = Instance.new("Frame", screenGui)
    f.Size = UDim2.new(0,340,0,78); f.BackgroundColor3 = C.bg_card
    f.BorderSizePixel = 0; f.ZIndex = 2000
    f.Position = UDim2.new(1,20,0,70); f.ClipsDescendants = true
    Instance.new("UICorner", f).CornerRadius = UDim.new(0,14)

    local bar = Instance.new("Frame", f)
    bar.Size = UDim2.new(0,4,1,0); bar.BackgroundColor3 = col
    bar.BorderSizePixel = 0; bar.ZIndex = 2002
    Instance.new("UICorner", bar).CornerRadius = UDim.new(0,4)

    local g2n = Instance.new("UIGradient", f)
    g2n.Color = ColorSequence.new{
    ...
    iconCircle.BackgroundColor3 = col; iconCircle.BackgroundTransparency = 0.75
    iconCircle.BorderSizePixel = 0; iconCircle.ZIndex = 2003
    Instance.new("UICorner", iconCircle).CornerRadius = UDim.new(1,0)
    addStroke(iconCircle, col, 1.5, 0.2)
    local iconLbl2 = Instance.new("TextLabel", iconCircle)
    iconLbl2.Size = UDim2.new(1,0,1,0); iconLbl2.BackgroundTransparency = 1
    iconLbl2.Text = getNotifIcon(col); iconLbl2.TextColor3 = col
    iconLbl2.Font = Enum.Font.GothamBold; iconLbl2.TextSize = 11; iconLbl2.ZIndex = 2004
    ...
    ml.Font = Enum.Font.Gotham; ml.TextSize = 12; ml.TextWrapped = true
    ml.TextXAlignment = Enum.TextXAlignment.Left; ml.ZIndex = 2002
...
local function makeGlow(sz, off, col, trans)
    ...
    g.Size = UDim2.new(0,HUB_W+sz,0,HUB_H+sz)
    g.Position = UDim2.new(0.5,0,0.5,off+15)
...
mainFrame.ZIndex = 1; mainFrame.Visible = false
...
task.spawn(function()
    while mainStroke.Parent do
        if mainStrokeGlowActive then
            tweenSine(mainStroke, {Transparency=0.5, Color=mainStrokeCurrentColor}, 1.2)
            task.wait(1.2)
            tweenSine(mainStroke, {Transparency=0, Color=mainStrokeCurrentColor}, 1.2)
            task.wait(1.2)
        else
            task.wait(0.5)
        end
...
tbGrad.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0,   Color3.fromRGB(12,40,22)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(14,26,18)),
    ColorSequenceKeypoint.new(1,   Color3.fromRGB(14,14,28)),
}; tbGrad.Rotation = 90

local sep = Instance.new("Frame", titleBar)
sep.Size = UDim2.new(0.6,0,0,1); sep.Position = UDim2.new(0.2,0,1,-2)
sep.BackgroundColor3 = C.accent_green; sep.BackgroundTransparency = 0.4
...
UserInputService.InputChanged:Connect(function(input)
    if input==dragInput and dragging then
        ...
        tw(mainFrame, {Position=UDim2.new(0,rx,0,ry)}, 0.06)
        glowDirty = true
    end
end)

local logoFrame = Instance.new("Frame", titleBar)
logoFrame.Size = UDim2.new(0,44,0,44); logoFrame.Position = UDim2.new(0,10,0,9)
logoFrame.BackgroundColor3 = C.accent_green; logoFrame.BackgroundTransparency = 0.75
logoFrame.BorderSizePixel = 0; logoFrame.ZIndex = 11
Instance.new("UICorner", logoFrame).CornerRadius = UDim.new(0,12)
addStroke(logoFrame, C.accent_green, 1.5, 0.3)
local logoIcon = Instance.new("TextLabel", logoFrame)
logoIcon.Size = UDim2.new(1,0,1,0); logoIcon.BackgroundTransparency = 1
logoIcon.Text = "[BF]"; logoIcon.TextColor3 = C.accent_green
logoIcon.Font = Enum.Font.GothamBold; logoIcon.TextSize = 16; logoIcon.ZIndex = 12

local titleLbl = Instance.new("TextLabel", titleBar)
titleLbl.Size = UDim2.new(1,-160,0,28); titleLbl.Position = UDim2.new(0,62,0,8)
titleLbl.BackgroundTransparency = 1; titleLbl.Text = "Build a Bamboo Factory"
titleLbl.TextColor3 = C.text_white; titleLbl.Font = Enum.Font.GothamBold
titleLbl.TextSize = 20; titleLbl.TextXAlignment = Enum.TextXAlignment.Left; titleLbl.ZIndex = 11
...
versionLbl.Size = UDim2.new(0,200,0,18); versionLbl.Position = UDim2.new(0,62,0,38)
versionLbl.BackgroundTransparency = 1; versionLbl.Text = "v2.4  |  BambooFactory Edition"
versionLbl.TextColor3 = C.text_muted; versionLbl.Font = Enum.Font.Gotham
versionLbl.TextSize = 11; versionLbl.TextXAlignment = Enum.TextXAlignment.Left; versionLbl.ZIndex = 11

local onlineBadge = Instance.new("Frame", titleBar)
onlineBadge.Size = UDim2.new(0,66,0,20); onlineBadge.Position = UDim2.new(0,270,0,38)
onlineBadge.BackgroundColor3 = C.accent_green; onlineBadge.BackgroundTransparency = 0.6
onlineBadge.BorderSizePixel = 0; onlineBadge.ZIndex = 11
...
onlineLbl.Text = "ONLINE"; onlineLbl.TextColor3 = C.accent_green
onlineLbl.Font = Enum.Font.GothamBold; onlineLbl.TextSize = 10; onlineLbl.ZIndex = 12
...
restoreBtn.Position = UDim2.new(1,-70,0,80)
restoreBtn.BackgroundColor3 = C.bg_dark
restoreBtn.BorderSizePixel = 0; restoreBtn.Visible = false; restoreBtn.ZIndex = 1000; restoreBtn.Active = true
Instance.new("UICorner", restoreBtn).CornerRadius = UDim.new(1,0)
local restoreStroke = addStroke(restoreBtn, C.accent_green, 2.5, 0.2)
...
restoreIcon.Text = "+"; restoreIcon.TextColor3 = C.accent_green
restoreIcon.TextSize = 36; restoreIcon.Font = Enum.Font.GothamBlack; restoreIcon.ZIndex = 1001

local restoreClick = Instance.new("TextButton", restoreBtn)
restoreClick.Size = UDim2.new(1,0,1,0); restoreClick.BackgroundTransparency = 1
restoreClick.Text = ""; restoreClick.ZIndex = 1002
addRipple(restoreClick)

restoreClick.MouseEnter:Connect(function()
    tw(restoreBtn,{BackgroundColor3=C.bg_card},0.2); tw(restoreIcon,{TextColor3=C.text_white},0.2)
end)
restoreClick.MouseLeave:Connect(function()
    tw(restoreBtn,{BackgroundColor3=C.bg_dark},0.2); tw(restoreIcon,{TextColor3=C.accent_green},0.2)
end)
...
local function hideHub()
    ...
    TweenService:Create(mainFrame,TweenInfo.new(0.45,Enum.EasingStyle.Back,Enum.EasingDirection.In),{
        Size=UDim2.new(0,0,0,0), Position=UDim2.new(1,-70,0,80), Rotation=-15,
    }):Play()
    for _,g in ipairs(allGlows) do
        TweenService:Create(g,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.In),{Size=UDim2.new(0,0,0,0)}):Play()
    end
    for _,v in ipairs(mainFrame:GetDescendants()) do
        pcall(function()
            if v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then tw(v,{TextTransparency=1},0.18) end
        end)
    end
    task.wait(0.45); mainFrame.Visible=false; mainFrame.Rotation=0
    for _,g in ipairs(allGlows) do g.Visible=false end
    mainStrokeGlowActive=false
...
local function showHub()
    ...
    mainFrame.Position=UDim2.new(0.5,0,0.5,0); mainFrame.Rotation=-15; mainStrokeGlowActive=true
    for _,g in ipairs(allGlows) do g.Visible=true; g.Size=UDim2.new(0,0,0,0) end
    TweenService:Create(mainFrame,TweenInfo.new(0.6,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Size=UDim2.new(0,HUB_W,0,HUB_H), Position=UDim2.new(0.5,0,0.5,0), Rotation=0,
    }):Play()
    for i,g in ipairs(allGlows) do
        local e=(i-1)*14+8
        TweenService:Create(g,TweenInfo.new(0.55,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,HUB_W+e,0,HUB_H+e)}):Play()
        task.wait(0.04)
    end
    syncGlows()
end
...
tabBar.Size = UDim2.new(1,0,0,40); tabBar.BackgroundColor3 = Color3.fromRGB(16,16,30)
...
tabLayout2.FillDirection = Enum.FillDirection.Horizontal; tabLayout2.Padding = UDim.new(0,3)
...
addStroke(tabIndicator,C.accent_green,1,0.3)
...
local function makeScrollFrame(parent)
    ...
    sf.ScrollBarImageColor3 = C.accent_green
...
local allFrames    = {collectFrame, buyFrame, cashFrame, configFrame}

local TAB_COLS   = {C.accent_green, C.accent_gold, C.accent_cyan, C.accent_purple}
local TAB_FRAMES = allFrames
    ...
    for i, child in ipairs(children) do
        ...
        task.delay((i-1) * 0.045, function()
            if child and child.Parent then
                tw(child, {BackgroundTransparency = child:IsA("ScrollingFrame") and 1 or 0}, 0.25, Enum.EasingStyle.Quad)
            end
        end)
    end
end
...
local function switchTab(order)
    ...
    local tbw=tabBar.AbsoluteSize.X; local tot=4*TAB_W+3*3; local left=(tbw-tot)/2
    tw(tabIndicator,{Size=UDim2.new(0,TAB_W*0.7,0,30)},0.12,Enum.EasingStyle.Quad)
    task.delay(0.12, function()
        tw(tabIndicator,{Position=UDim2.new(0,left+(order-1)*(TAB_W+3),0,5)},0.28,Enum.EasingStyle.Back)
...
for _,def in ipairs({{"Collect","[C]",1},{"AutoBuy","[B]",2},{"AutoCash","[$]",3},{"Config","[S]",4}}) do
    ...
    btn.TextSize = 12; btn.BorderSizePixel = 0; btn.ZIndex = 2
...
local function makeToggleCard(parent, labelText, icon, color, order)
    ...
    iconLbl.Font = Enum.Font.GothamBold; iconLbl.TextSize = 14; iconLbl.TextColor3 = C.text_muted
...
-- ════════════════════════════════════════════════════════════════
-- VARIAVEIS DAS ABAS (forward declarations)
-- ════════════════════════════════════════════════════════════════
...
-- v2.3: referencias aos sync functions dos toggles de automacao
local acSyncFn = function() end  -- AutoCollect
local abSyncFn = function() end  -- AutoBuy
local cashSyncFn = function() end -- AutoCash
...
local function lockCameraDown()
    ...
    applyAngle()
    camLockConn = RunService.RenderStepped:Connect(applyAngle)
end
...
local function setAutoCollect(state)
    ...
    if state then
        ...
        autoCollectThread = task.spawn(function()
            ...
            Placements.ChildRemoved:Connect(function(child)
                for i, c in ipairs(collectors) do if c == child then table.remove(collectors, i); break end end
            end)
            ...
            while autoCollectEnabled do
                for _,child in ipairs(collectors) do
                    ...
                    cp=rootPart:FindFirstChild("CollectPrompt")
                    if not cp or not cp.Enabled then continue end
...
-- v2.3: sync function para preset
...
local function getStock(itemName)
    return merchantStock[itemName] or -1
end
        ...
        autoBuyThread = task.spawn(function()
            ...
            while autoBuyEnabled do
                ...
                for _, entry in ipairs(autoBuyItems) do
                    ...
                    local stock = getStock(realName)
                    if stock == 0 then continue end
                    MerchantBuy:FireServer(realName)
                    task.wait(math.max(buyDelay, MIN_DELAY))
                end
                task.wait(0.5)
            end
        end)
...
-- Card: lista de itens salvos
local itemsCard = makeCard(buyFrame, 80, 3)
local ics = itemsCard:FindFirstChildWhichIsA("UIStroke")
if ics then tw(ics,{Color=C.accent_gold},0) end
local itemsLbl = Instance.new("TextLabel",itemsCard)
itemsLbl.Size=UDim2.new(1,-16,0,20); itemsLbl.Position=UDim2.new(0,10,0,6)
itemsLbl.BackgroundTransparency=1; itemsLbl.Text="Itens salvos"
itemsLbl.TextColor3=C.accent_gold; itemsLbl.Font=Enum.Font.GothamBold; itemsLbl.TextSize=13
...
addStroke(itemsScroll,C.border_dim,1,0.3)
...
isp.PaddingTop=UDim.new(0,5); isp.PaddingBottom=UDim.new(0,5)
isp.PaddingLeft=UDim.new(0,6); isp.PaddingRight=UDim.new(0,6)
itemsLayout2:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    local h = itemsLayout2.AbsoluteContentSize.Y + 14
    itemsCard.Size  = UDim2.new(1,-6,0, math.max(80, h+46))
    itemsScroll.Size= UDim2.new(1,-16,0, math.min(h, 140))
end)
...
local function closeSuggestions()
    ...
    selectedSuggestions = {}
    if addBtn then addBtn.Text = "Salvar" end
end
...
local function addItemTag(entry)
    ...
    addStroke(tag, C.border_dim, 1, 0.4)
    ...
    tagLbl.TextSize = 12; tagLbl.Font = Enum.Font.GothamBold; tagLbl.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local function updateModeBtn()
        if entry.flex then
            modeBtn.Text="Flex"; modeBtn.TextColor3=C.text_white; modeBtn.BackgroundColor3=Color3.fromRGB(30,80,180)
            local s=modeBtn:FindFirstChildWhichIsA("UIStroke"); if s then s.Color=C.accent_blue end
        else
            modeBtn.Text="Exato"; modeBtn.TextColor3=C.text_white; modeBtn.BackgroundColor3=Color3.fromRGB(150,70,10)
...
local inputCardStroke = inputCard:FindFirstChildWhichIsA("UIStroke")
if inputCardStroke then tw(inputCardStroke,{Color=C.accent_gold},0) end
local inputRow = Instance.new("Frame",inputCard)
inputRow.Size=UDim2.new(1,-16,0,30); inputRow.Position=UDim2.new(0,8,0,8)
inputRow.BackgroundTransparency=1
local irLayout2=Instance.new("UIListLayout",inputRow)
irLayout2.FillDirection=Enum.FillDirection.Horizontal; irLayout2.Padding=UDim.new(0,6)
irLayout2.VerticalAlignment=Enum.VerticalAlignment.Center
...
textBox.Text=""; textBox.TextColor3=C.text_white; textBox.TextSize=12; textBox.Font=Enum.Font.Gotham
...
addStroke(addBtn,C.accent_green,1,0.4); addRipple(addBtn)
...
local function updateModeToggle()
    if addFlexMode then
        modeToggleBtn.Text="[Flex]"; modeToggleBtn.TextColor3=C.text_white; modeToggleBtn.BackgroundColor3=Color3.fromRGB(30,80,180)
        tw(modeToggStr,{Color=C.accent_blue},0.15)
    else
        modeToggleBtn.Text="[Exato]"; modeToggleBtn.TextColor3=C.text_white; modeToggleBtn.BackgroundColor3=Color3.fromRGB(150,70,10)
        tw(modeToggStr,{Color=C.accent_gold},0.15)
    end
end
updateModeToggle()
modeToggleBtn.MouseButton1Click:Connect(function()
    addFlexMode=not addFlexMode; updateModeToggle()
    tweenBounce(modeToggleBtn,{TextSize=12},0.12); task.wait(0.25); tw(modeToggleBtn,{TextSize=10},0.15)
end)
...
flexHint.TextColor3=C.text_muted; flexHint.Font=Enum.Font.Gotham
flexHint.TextSize=9; flexHint.TextXAlignment=Enum.TextXAlignment.Left

suggestCard = makeCard(buyFrame, 0, 5)
suggestCard.BackgroundColor3 = Color3.fromRGB(16,16,30)
local suggestStroke = suggestCard:FindFirstChildWhichIsA("UIStroke")
if suggestStroke then tw(suggestStroke,{Color=C.accent_gold},0) end
...
suggestScroll.CanvasSize=UDim2.new(0,0,0,0); suggestScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y
suggestScroll.ScrollingDirection=Enum.ScrollingDirection.Y
...
suggestPad.PaddingTop=UDim.new(0,2); suggestPad.PaddingBottom=UDim.new(0,2)
suggestPad.PaddingLeft=UDim.new(0,2); suggestPad.PaddingRight=UDim.new(0,2)
...
local function updateSuggestions(query)
    ...
    for idx, nm in ipairs(matches) do
        ...
        chk.BorderSizePixel=0; chk.BackgroundColor3=Color3.fromRGB(30,30,50)
        Instance.new("UICorner",chk).CornerRadius=UDim.new(0,4)
        addStroke(chk, C.border_dim, 1, 0.2)
        ...
        lbl.BackgroundTransparency=1; lbl.Text=nm
        ...
        if selectedSuggestions[nm] then
            chkMark.Text="v"; chk.BackgroundColor3=Color3.fromRGB(30,80,180)
            local s=chk:FindFirstChildWhichIsA("UIStroke"); if s then s.Color=C.accent_gold end
        end
        ...
        rowBtn.MouseEnter:Connect(function()
            if not selectedSuggestions[nm] then
                tw(row,{BackgroundColor3=Color3.fromRGB(30,30,54)},0.1); tw(lbl,{TextColor3=Color3.fromRGB(200,200,230)},0.1)
            end
        end)
        rowBtn.MouseLeave:Connect(function()
            if not selectedSuggestions[nm] then
                tw(row,{BackgroundColor3=Color3.fromRGB(22,22,40)},0.1); tw(lbl,{TextColor3=C.text_dim},0.1)
            end
        end)
...
textBox.Focused:Connect(function()
    local now = os.clock()
    if now - wsCacheTime > 30 then
        task.spawn(function() wsNames=scanWorkspaceNames(); wsCacheTime=os.clock() end)
    end
...
addBtn.MouseButton1Click:Connect(function()
    ...
    if hasSelected then
        for nm in pairs(selectedSuggestions) do
            ...
            if isDupe then duplicates=duplicates+1
            else
                local entry={name=nm,flex=addFlexMode}
                table.insert(autoBuyItems,entry); addItemTag(entry); added=added+1
            end
        end
        saveAutoBuyItemsFull(autoBuyItems); textBox.Text=""; closeSuggestions()
        if added > 0 then
            local modeStr=addFlexMode and "Flex" or "Exato"
            notify("AutoBuy","+"..added.." itens ["..modeStr.."]"..(duplicates>0 and " ("..duplicates.." ja existiam)" or ""),3,C.accent_gold)
            tweenBounce(addBtn,{TextSize=14},0.15); task.wait(0.3); tw(addBtn,{TextSize=12},0.2)
        else
            tweenBounce(textBox,{BackgroundColor3=Color3.fromRGB(60,20,20)},0.1)
            task.wait(0.4); tw(textBox,{BackgroundColor3=C.bg_input},0.3)
        end
    else
        ...
        for _,v in ipairs(autoBuyItems) do
            if v.name==name then
                tweenBounce(textBox,{BackgroundColor3=Color3.fromRGB(60,20,20)},0.1)
                task.wait(0.4); tw(textBox,{BackgroundColor3=C.bg_input},0.3); return
            end
        end
        local entry={name=name,flex=addFlexMode}
        table.insert(autoBuyItems,entry); saveAutoBuyItemsFull(autoBuyItems); addItemTag(entry)
        textBox.Text=""; closeSuggestions()
        tweenBounce(addBtn,{TextSize=14},0.15); task.wait(0.3); tw(addBtn,{TextSize=12},0.2)
        local modeStr=addFlexMode and "Flex" or "Exato"
        notify("AutoBuy","["..modeStr.."] "..name,2,C.accent_gold)
    end
end)
...
local dcs = debugCard:FindFirstChildWhichIsA("UIStroke")
if dcs then tw(dcs,{Color=C.accent_purple},0) end
...
debugHeader.TextColor3=C.accent_purple; debugHeader.Font=Enum.Font.GothamBold
debugHeader.TextSize=11; debugHeader.TextXAlignment=Enum.TextXAlignment.Left
...
debugLbl.TextColor3=C.text_muted; debugLbl.Font=Enum.Font.Gotham
debugLbl.TextSize=11; debugLbl.TextWrapped=true; debugLbl.TextXAlignment=Enum.TextXAlignment.Left
...
copyDebugBtn.Text="Copiar nome capturado pro campo de busca"
copyDebugBtn.TextColor3=C.accent_purple; copyDebugBtn.Font=Enum.Font.GothamBold; copyDebugBtn.TextSize=10
Instance.new("UICorner",copyDebugBtn).CornerRadius=UDim.new(0,7)
addStroke(copyDebugBtn,C.accent_purple,1,0.4); addRipple(copyDebugBtn)
...
copyDebugBtn.MouseButton1Click:Connect(function()
    if lastCapturedName and lastCapturedName~="" then
        textBox.Text=lastCapturedName; switchTab(2)
        notify("Debug","Nome copiado: "..lastCapturedName,2,C.accent_purple)
...
-- v2.3: getCurrentData agora inclui estado dos toggles
local function getCurrentData()
    return {
        ...
        -- v2.3: estado dos toggles de automacao
...
-- v2.3: applyProfileData agora aplica toggles de automacao
local function applyProfileData(data)
    ...
    if data.jpower then
        ...
        if jumpInputRef then jumpInputRef.Text=tostring(jpower) end
    end
    ...
    -- v2.3: aplica estado dos toggles
...
local function makeConfigToggle(parent, label, icon, initial, order, color, onChange)
    ...
    kn2.Size=UDim2.new(0,24,0,24)
    kn2.Position=initial and UDim2.new(1,-27,0,3) or UDim2.new(0,3,0,3)
    kn2.BackgroundColor3=C.text_white; kn2.BorderSizePixel=0
    ...
    local function upd2(v)
        state.value=v
        local c3=v and color or C.off_color
        tw(tr2,{BackgroundColor3=c3},0.25)
        tweenBack(kn2,{Position=v and UDim2.new(1,-27,0,3) or UDim2.new(0,3,0,3)},0.28)
        ic2.TextColor3=v and color or C.text_muted
        if cs2 then tw(cs2,{Color=v and color or C.border_dim},0.25) end
    end
...
local function buildConfigTab()
    ...
    -- ── CARD: PRESETS ──────────────────────────────────────────
    -- Altura total aumentada para comportar a secao de autoload (v2.3)
    local profCard = makeCard(configFrame, 420, 1)
    local profStroke = profCard:FindFirstChildWhichIsA("UIStroke")
    if profStroke then tw(profStroke,{Color=C.accent_gold},0) end
    ...
    profHeader.TextColor3=C.accent_gold; profHeader.Font=Enum.Font.GothamBold
    profHeader.TextSize=14; profHeader.TextXAlignment=Enum.TextXAlignment.Left
    ...
    currentProfLbl.TextColor3=C.text_dim; currentProfLbl.Font=Enum.Font.Gotham
    currentProfLbl.TextSize=11; currentProfLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Botao: Salvar no perfil ativo
    ...
    saveActiveBtn.Text="[S] Salvar no Ativo"; saveActiveBtn.TextColor3=C.accent_green
    saveActiveBtn.Font=Enum.Font.GothamBold; saveActiveBtn.TextSize=10
    Instance.new("UICorner",saveActiveBtn).CornerRadius=UDim.new(0,7)
    addStroke(saveActiveBtn,C.accent_green,1,0.4); addRipple(saveActiveBtn)
    ...
    saveActiveBtn.MouseButton1Click:Connect(function()
        ...
        local origText=saveActiveBtn.Text; saveActiveBtn.Text="Salvo!"
        tw(saveActiveBtn,{BackgroundColor3=Color3.fromRGB(20,160,80)},0.15)
    ...
    profScroll.CanvasSize=UDim2.new(0,0,0,0); profScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y
    profScroll.ScrollingDirection=Enum.ScrollingDirection.Y
    Instance.new("UICorner",profScroll).CornerRadius=UDim.new(0,9)
    addStroke(profScroll,C.border_dim,1,0)
    ...
    -- Campo de nome do preset
    local nameInput = Instance.new("TextBox",profCard)
    nameInput.Size=UDim2.new(1,-20,0,30); nameInput.Position=UDim2.new(0,10,0,158)
    nameInput.BackgroundColor3=C.bg_input; nameInput.TextColor3=C.text_white
    nameInput.Font=Enum.Font.Gotham; nameInput.TextSize=13
    nameInput.PlaceholderText="Nome do preset..."; nameInput.PlaceholderColor3=C.text_muted
    nameInput.Text=""; nameInput.ClearTextOnFocus=false; nameInput.BorderSizePixel=0
    ...
    -- 3 botoes de acao: Carregar | Criar | Deletar
    local function makeSmallBtn(parent, label, color, xScale, xOffset, yOff, onClick)
        ...
        addStroke(btn,color,1,0.4); addRipple(btn)
    ...
    makeSmallBtn(profCard,"Carregar",C.accent_blue, 0, 10, 196, function()
        ...
        if data then applyProfileData(data); updProfList()
        else notify("Preset","Perfil nao encontrado!",2,C.accent_red) end
    end)
    makeSmallBtn(profCard,"Criar", C.accent_green, 0.33, 5, 196, function()
        local n=nameInput.Text:match("^%s*(.-)%s*$"); if n=="" then return end
        if ProfileSystem:createProfile(n) then
            -- Salva estado atual no novo preset
            ProfileSystem.currentProfile=n
            ProfileSystem:saveCurrentProfile(getCurrentData())
            if currentProfLbl then currentProfLbl.Text="Ativo: "..n end
            updProfList(); notify("Preset","Perfil '"..n.."' criado com estado atual!",2,C.accent_green)
    ...
    makeSmallBtn(profCard,"Deletar", C.accent_red, 0.66, 5, 196, function()
        ...
        if ProfileSystem:deleteProfile(n) then
            if currentProfLbl then currentProfLbl.Text="Ativo: "..ProfileSystem.currentProfile end
            -- Se o preset deletado era o autoload, limpa o autoload
            local al = loadAutoload()
            if al == n then saveAutoload(nil); if autoloadLbl then autoloadLbl.Text="Sem autoload definido" end end
    ...
    tipLbl.Size=UDim2.new(1,-20,0,26); tipLbl.Position=UDim2.new(0,10,0,232)
    tipLbl.BackgroundTransparency=1
    tipLbl.Text="Click = preenche nome  |  Click direito = carrega direto"
    tipLbl.TextColor3=C.text_muted; tipLbl.Font=Enum.Font.Gotham
    tipLbl.TextSize=10; tipLbl.TextWrapped=true; tipLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Botao: Salvar preset atual (salva pelo nome do campo ou pelo perfil ativo)
    ...
    savePresetBtn.Text="Salvar preset atual com estado dos toggles"; savePresetBtn.TextColor3=C.accent_green
    savePresetBtn.Font=Enum.Font.GothamBold; savePresetBtn.TextSize=11
    Instance.new("UICorner",savePresetBtn).CornerRadius=UDim.new(0,8)
    addStroke(savePresetBtn,C.accent_green,1,0.4); addRipple(savePresetBtn)
    ...
    savePresetBtn.MouseButton1Click:Connect(function()
        local n=nameInput.Text:match("^%s*(.-)%s*$")
        if n~="" then
            if not ProfileSystem.profiles[n] then ProfileSystem:createProfile(n) end
            ProfileSystem.currentProfile=n
        end
        ProfileSystem:saveCurrentProfile(getCurrentData())
        if currentProfLbl then currentProfLbl.Text="Ativo: "..ProfileSystem.currentProfile end
        updProfList()
        notify("Preset","Salvo como '"..ProfileSystem.currentProfile.."' (com toggles)!",2,C.accent_green)
    end)

    -- ── SECAO AUTOLOAD (v2.3) ──────────────────────────────────
    local alSep = Instance.new("Frame",profCard)
    alSep.Size=UDim2.new(1,-24,0,1); alSep.Position=UDim2.new(0,12,0,292)
    alSep.BackgroundColor3=C.border_dim; alSep.BorderSizePixel=0
    local alSepG=Instance.new("UIGradient",alSep)
    alSepG.Color=ColorSequence.new{
        ColorSequenceKeypoint.new(0,Color3.fromRGB(0,0,0)),
    ...
    alTitle.Size=UDim2.new(1,-16,0,20); alTitle.Position=UDim2.new(0,12,0,300)
    alTitle.BackgroundTransparency=1; alTitle.Text="[>>] Autoload ao Executar"
    alTitle.TextColor3=C.accent_gold; alTitle.Font=Enum.Font.GothamBold
    alTitle.TextSize=12; alTitle.TextXAlignment=Enum.TextXAlignment.Left

    -- Label mostrando qual preset esta definido como autoload
    local currentAutoload = loadAutoload()
    autoloadLbl = Instance.new("TextLabel",profCard)
    autoloadLbl.Size=UDim2.new(1,-20,0,16); autoloadLbl.Position=UDim2.new(0,12,0,322)
    ...
    autoloadLbl.TextColor3 = currentAutoload and C.accent_green or C.text_muted
    autoloadLbl.Font=Enum.Font.Gotham; autoloadLbl.TextSize=11
    autoloadLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Botao: Definir preset do campo como autoload
    ...
    setAutoloadBtn.Text="[>>] Definir como Autoload"; setAutoloadBtn.TextColor3=C.accent_blue
    setAutoloadBtn.Font=Enum.Font.GothamBold; setAutoloadBtn.TextSize=10
    Instance.new("UICorner",setAutoloadBtn).CornerRadius=UDim.new(0,8)
    addStroke(setAutoloadBtn,C.accent_blue,1,0.4); addRipple(setAutoloadBtn)
    ...
    setAutoloadBtn.MouseButton1Click:Connect(function()
        local n=nameInput.Text:match("^%s*(.-)%s*$")
        -- Se o campo estiver vazio, usa o perfil ativo
        if n=="" then n=ProfileSystem.currentProfile end
        if not ProfileSystem.profiles[n] then
            notify("Autoload","Preset '"..n.."' nao existe!",2,C.accent_red); return
        end
        saveAutoload(n)
        autoloadLbl.Text="Autoload: "..n; tw(autoloadLbl,{TextColor3=C.accent_green},0.3)
        tweenBounce(setAutoloadBtn,{TextSize=12},0.15); task.wait(0.3); tw(setAutoloadBtn,{TextSize=10},0.2)
        notify("Autoload","Preset '"..n.."' vai carregar automaticamente!",3,C.accent_blue)
    end)

    -- Botao: Remover autoload
    ...
    clearAutoloadBtn.Text="[x] Remover"; clearAutoloadBtn.TextColor3=C.accent_red
    clearAutoloadBtn.Font=Enum.Font.GothamBold; clearAutoloadBtn.TextSize=10
    Instance.new("UICorner",clearAutoloadBtn).CornerRadius=UDim.new(0,8)
    addStroke(clearAutoloadBtn,C.accent_red,1,0.4); addRipple(clearAutoloadBtn)
    ...
    clearAutoloadBtn.MouseButton1Click:Connect(function()
        saveAutoload(nil)
        autoloadLbl.Text="Sem autoload definido"; tw(autoloadLbl,{TextColor3=C.text_muted},0.3)
        notify("Autoload","Autoload removido.",2,C.off_color)
    end)

    local alHint=Instance.new("TextLabel",profCard)
    alHint.Size=UDim2.new(1,-20,0,30); alHint.Position=UDim2.new(0,12,0,374)
    alHint.BackgroundTransparency=1
    alHint.Text="O preset definido como autoload carrega automaticamente os toggles e configs ao executar o script."
    alHint.TextColor3=C.text_muted; alHint.Font=Enum.Font.Gotham
    alHint.TextSize=9; alHint.TextWrapped=true; alHint.TextXAlignment=Enum.TextXAlignment.Left

    -- ── FUNCAO updProfList (v2.3: mostra badge de autoload) ────
    function updProfList()
        ...
        for idx,name in ipairs(ProfileSystem:getNames()) do
            local isA=(name==ProfileSystem.currentProfile)
            local isAL=(name==alName)
            local row=Instance.new("Frame",profScroll)
            row.Size=UDim2.new(1,0,0,28); row.LayoutOrder=idx
            row.BackgroundColor3=isA and Color3.fromRGB(30,55,110) or Color3.fromRGB(22,22,38)
            row.BorderSizePixel=0
            Instance.new("UICorner",row).CornerRadius=UDim.new(0,7)
            if isA then addStroke(row,C.accent_gold,1.5,0.3) end

            -- Icone/texto do nome
            local btn2=Instance.new("TextButton",row)
            btn2.Size=UDim2.new(1,isAL and -58 or -8,1,0); btn2.Position=UDim2.new(0,0,0,0)
            btn2.BackgroundTransparency=1; btn2.ZIndex=2
            btn2.Text=(isA and ">> " or "   ")..name
            btn2.TextColor3=isA and C.accent_gold or C.text_dim
            btn2.Font=Enum.Font.GothamBold; btn2.TextSize=12; btn2.TextXAlignment=Enum.TextXAlignment.Left
            local pp=Instance.new("UIPadding",btn2); pp.PaddingLeft=UDim.new(0,8)
            addRipple(btn2)
            ...
            btn2.MouseButton1Click:Connect(function()
                nameInput.Text=name
                tweenBounce(btn2,{TextColor3=C.text_white},0.15)
                task.wait(0.35); tw(btn2,{TextColor3=isA and C.accent_gold or C.text_dim},0.3)
            end)
            btn2.MouseButton2Click:Connect(function()
                local data=ProfileSystem:loadProfile(name)
                if data then applyProfileData(data); updProfList() end
            end)
            ...
            if isAL then
                ...
                Instance.new("UICorner",alBadge).CornerRadius=UDim.new(0,5)
                addStroke(alBadge,C.accent_blue,1,0.3)
                local alLbl=Instance.new("TextLabel",alBadge)
                alLbl.Size=UDim2.new(1,0,1,0); alLbl.BackgroundTransparency=1
                alLbl.Text="AUTO"; alLbl.TextColor3=C.accent_blue
                alLbl.Font=Enum.Font.GothamBold; alLbl.TextSize=9
            end
        end
    end
    updProfList()
    ...
    -- ── TPWALK ────────────────────────────────────────────────
    local twCard = makeCard(configFrame,62,2)
    local twStroke = twCard:FindFirstChildWhichIsA("UIStroke")
    if twStroke then tw(twStroke,{Color=C.accent_blue},0) end
    local twLabel=Instance.new("TextLabel",twCard)
    twLabel.Size=UDim2.new(1,-16,0,20); twLabel.Position=UDim2.new(0,12,0,6)
    twLabel.BackgroundTransparency=1; twLabel.Text="[>]  TPWalk  -  Velocidade de movimento"
    twLabel.TextColor3=C.accent_blue; twLabel.Font=Enum.Font.GothamBold
    twLabel.TextSize=12; twLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    tpwalkInputRef.Font=Enum.Font.GothamBold; tpwalkInputRef.TextSize=13; tpwalkInputRef.BorderSizePixel=0
    tpwalkInputRef.ClearTextOnFocus=false; tpwalkInputRef.Text=tostring(tpwalkSpeed)
    ...
    tpwalkInputRef.FocusLost:Connect(function()
        ...
        if v and v>0 then tpwalkSpeed=v; savedSettings["tpwalk_speed"]=v; saveSettings(savedSettings)
            notify("TPWalk","Velocidade: "..v,2,C.accent_blue)
        else tpwalkInputRef.Text=tostring(tpwalkSpeed) end
    end)

    -- ── JUMP POWER ────────────────────────────────────────────
    local jpCard=makeCard(configFrame,62,3)
    local jpStroke=jpCard:FindFirstChildWhichIsA("UIStroke")
    if jpStroke then tw(jpStroke,{Color=C.accent_cyan},0) end
    local jpLabel=Instance.new("TextLabel",jpCard)
    jpLabel.Size=UDim2.new(1,-16,0,20); jpLabel.Position=UDim2.new(0,12,0,6)
    jpLabel.BackgroundTransparency=1; jpLabel.Text="[^]  JumpPower  -  Forca do pulo"
    jpLabel.TextColor3=C.accent_cyan; jpLabel.Font=Enum.Font.GothamBold
    jpLabel.TextSize=12; jpLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    jumpInputRef.Font=Enum.Font.GothamBold; jumpInputRef.TextSize=13; jumpInputRef.BorderSizePixel=0
    jumpInputRef.ClearTextOnFocus=false; jumpInputRef.Text=tostring(jpower)
    ...
    jumpInputRef.FocusLost:Connect(function()
        ...
        if v and v>0 then
            jpower=v
            if humanoid.UseJumpPower then humanoid.JumpPower=jpower else humanoid.JumpHeight=jpower end
            savedSettings["jpower"]=v; saveSettings(savedSettings)
            notify("JumpPower","Forca: "..v,2,C.accent_cyan)
    ...
    local ijUpd = makeConfigToggle(configFrame,"InfJump","[^]",infJumpEnabled,4,C.accent_cyan,function(v)
        infJumpEnabled=v
        notify(v and "InfJump ON" or "InfJump OFF","",2,v and C.accent_cyan or C.off_color)
    end)
    infJumpSyncFn=function(v) infJumpEnabled=v; ijUpd(v) end

    -- ── ANTIAFK ───────────────────────────────────────────────
    local afkCard=makeCard(configFrame,52,5)
    local afkS=afkCard:FindFirstChildWhichIsA("UIStroke")
    if afkS then tw(afkS,{Color=C.accent_purple},0) end
    local afkBtn=Instance.new("TextButton",afkCard)
    afkBtn.Size=UDim2.new(1,-16,0,38); afkBtn.Position=UDim2.new(0,8,0,7)
    afkBtn.BackgroundColor3=C.accent_purple; afkBtn.BackgroundTransparency=0.15
    afkBtn.Text="[A]  Ativar AntiAFK"; afkBtn.TextColor3=C.text_white
    ...
    addStroke(afkBtn,C.accent_purple,1.5,0.4); addRipple(afkBtn)
    ...
    afkBtn.MouseButton1Click:Connect(function()
        if not antiAfkEnabled then
            antiAfkEnabled=true
            notify("AntiAFK","Ativando script AntiAFK...",2,C.accent_purple)
            task.spawn(function()
                ...
                if ok then
                    ...
                    tw(afkBtn,{BackgroundColor3=Color3.fromRGB(80,20,120),BackgroundTransparency=0},0.3)
                    afkBtn.Text="[A]  AntiAFK ATIVO"
                else antiAfkEnabled=false; notify("AntiAFK","Falha ao carregar!",3,C.accent_red) end
            end)
        else notify("AntiAFK","Ja esta ativo!",2,C.accent_orange) end
    end)

    -- ── DELAYS ────────────────────────────────────────────────
    local delayCard=makeCard(configFrame,130,6)
    local ds=delayCard:FindFirstChildWhichIsA("UIStroke")
    if ds then tw(ds,{Color=C.accent_purple},0) end
    local delayHeader=Instance.new("TextLabel",delayCard)
    delayHeader.Size=UDim2.new(1,-16,0,20); delayHeader.Position=UDim2.new(0,12,0,6)
    delayHeader.BackgroundTransparency=1; delayHeader.Text="Delays das automacoes (segundos)"
    delayHeader.TextColor3=C.accent_purple; delayHeader.Font=Enum.Font.GothamBold
    delayHeader.TextSize=12; delayHeader.TextXAlignment=Enum.TextXAlignment.Left
    local function makeDelayInput(parent, label, color, yOff, defaultVal, onFocusLost)
        local lbl2=Instance.new("TextLabel",parent)
        lbl2.Size=UDim2.new(0.42,0,0,24); lbl2.Position=UDim2.new(0,12,0,yOff)
        lbl2.BackgroundTransparency=1; lbl2.Text=label; lbl2.TextColor3=color
        lbl2.Font=Enum.Font.GothamBold; lbl2.TextSize=12; lbl2.TextXAlignment=Enum.TextXAlignment.Left
        local box2=Instance.new("TextBox",parent)
        box2.Size=UDim2.new(0.5,-6,0,24); box2.Position=UDim2.new(0.5,-2,0,yOff)
        box2.BackgroundColor3=C.bg_input; box2.TextColor3=C.text_white
        box2.PlaceholderColor3=C.text_muted; box2.Font=Enum.Font.GothamBold
        box2.TextSize=13; box2.BorderSizePixel=0; box2.ClearTextOnFocus=false; box2.Text=tostring(defaultVal)
    ...
    addStroke(stopBtn,C.accent_red,1.5,0.4); addRipple(stopBtn)
    ...
    stopBtn.MouseButton1Click:Connect(function()
        setAutoCollect(false); setAutoBuy(false); setAutoCash(false)
        infJumpEnabled=false; ijUpd(false)
        tweenBounce(stopBtn,{BackgroundColor3=Color3.fromRGB(255,100,100)},0.1)
        task.wait(0.5); tw(stopBtn,{BackgroundColor3=C.accent_red},0.3)
...
player.CharacterAdded:Connect(function(newChar)
    ...
    applyJump(); humanoid:GetPropertyChangedSignal("JumpPower"):Connect(applyJump)
    setupTpwalk(newChar)
...
-- ════════════════════════════════════════════════════════════════
-- SEQUENCIA DE LOADING -> abre hub -> autoload (v2.3)
-- ════════════════════════════════════════════════════════════════
local function runLoadingSequence(onComplete)
    task.spawn(function()
        task.wait(0.2)
        tw(lsTitle,{TextTransparency=0},0.8); task.wait(0.3)
        tw(lsVer,{TextTransparency=0},0.8)
    end)
    local steps = {
        ...
        {text="Pronto! [BF]",                  prog=1.00, wait=0.4},
    }
    task.spawn(function()
        for _,step in ipairs(steps) do
            lsStatus.Text=step.text
            tw(lsBar,{Size=UDim2.new(step.prog,0,1,0)},0.4,Enum.EasingStyle.Quad)
            task.wait(step.wait)
        end
        ...
        task.wait(0.65); loadScreen.Visible=false
        onComplete()
    end)
end
...
runLoadingSequence(function()
    ...
    task.wait(0.08)
    for i,g in ipairs(allGlows) do
        local e=(i-1)*14+8; tweenBack(g,{Size=UDim2.new(0,HUB_W+e,0,HUB_H+e)},0.5)
        task.wait(0.04)
    end
    syncGlows(); task.wait(0.5)
    tw(mainStroke,{Transparency=0.7},0.2); task.wait(0.2); tw(mainStroke,{Transparency=0},0.3)
    notify("Build a Bamboo Factory Hub","v2.3 carregado com sucesso!",3,C.accent_green)

    -- v2.3: aplica autoload se definido
    task.delay(0.6, function()
        local alName = loadAutoload()
        if alName and ProfileSystem.profiles[alName] then
            lsStatus.Text = "Aplicando autoload: "..alName
            local data = ProfileSystem:loadProfile(alName)
            if data then
                applyProfileData(data)
                updProfList()
                notify("Autoload","Preset '"..alName.."' carregado automaticamente!",3,C.accent_blue)
            end
        end
    end)
end)

print("Build a Bamboo Factory Hub v2.3 -- Carregado!")
